...
internal class DoujinDesuFunParser(context: MangaLoaderContext) :
    ...
    override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
        ...
        val imagesArray = json.jsonObject["data"]
            ?.jsonObject?.get("chapter")
            ?.jsonObject?.get("images")
            ?.jsonArray ?: throw IOException("No images found in API response")

        return imagesArray.map { imageJson ->
            val imageUrl = imageJson.jsonPrimitive.content
            MangaPage(
